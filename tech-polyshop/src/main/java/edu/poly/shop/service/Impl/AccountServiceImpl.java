...
public Account login(String username, String password) {
	 ...
	 //optExit.isPresent() là tồn tại
	 // matches so sánh xem nhập  có giống với mk theo trong sql k 
	 
	 if(optExit.isPresent() && bCryptPasswordEncoder.matches(password, optExit.get().getPassword())) {
...
public <S extends Account> S save(S entity) {
	...
	Optional<Account> otpExit = findById(entity.getUsername());
	
	if(otpExit.isPresent()) {
		// k nhập passworf mới khi cấp nhật thì lấy lại password cũ
		if(StringUtils.isEmpty(entity.getPassword())) {
			entity.setPassword(otpExit.get().getPassword());
		}else {
